...
public class GamePricingForm extends JPanel {
	...
	private static final Logger LOGGER = Logger.getLogger(GamePricingForm.class.getName()); 
	// Components
...
//		return new JScrollPane(tablePanel);
		...
		tableModel.setPricings(pricings);
	...
	private JPanel createInputPanel() {
		...
		List<Game> games = gameRepository.readAll();
				...
				deleteButton.addActionListener(this::deleteSelectedGamePricing);  
	...
	void initGamePricingForm(){
		 ...
		 pricingldField.setText("" );
	        ...
	        LocalDateTime eightMonthsLater = currentDate.plus(5, ChronoUnit.YEARS);
	...
	private void deleteSelectedGamePricing(ActionEvent event) {
		int row = pricingTable.getSelectedRow();
		LOGGER.info("Game-Pricing selected row N° : "+row);
		if (row >= 0) {
			try {
				GamePricing gamePricing = tableModel.getGamePricingAt(row);
				LOGGER.info("Game-Pricing price  : "+gamePricing.getPrice());
				boolean deleted = gamePricingRepository.deleteGamePricing(gamePricing.getPricingId());
				if (deleted) {
					tableModel.removePricing(row);
					LOGGER.info("Game-Pricing association deleted successfully.");
				}
			} catch (Exception e) {
				LOGGER.log(Level.SEVERE, "Error deleting game-Pricing association: " + e.getMessage(), e);
				JOptionPane.showMessageDialog(this, "Error deleting game-Pricing association: " + e.getMessage(),
						"Error", JOptionPane.ERROR_MESSAGE);
			}
		}else {
			LOGGER.info("Game-Pricing no selection  row N° : "+row);
		}
